...
export class CaroselComponent implements OnInit {
  ...
  public setCenter(): void {
    ...
    let actualPosition: number =
      parseInt(this.centerTarget) + this.pageCount - 1;
    let actualCenter = this.carouselElementRails.querySelector(
      '.carousel\\.center'
    );
    actualCenter.className = actualCenter.className
      .replace('carousel.center', '')
      .trim();
  ...
  private next(): void {
    this.pageCount += 1;
    if (
      this.pageCount >= this.carouselElementItems.length ||
      this.pageCount >
        this.carouselElementItems.length - parseInt(this.itemsByPages)
    )
      this.pageCount = 0;
    this.setCenter();
    this.carouselElementRails.style.transform = `translateX(-${
      this.pageCount * this.pageDimension
    }px)`;
  }
  ...
  public trackEvents(): void {
    window.document.querySelector(this.nextTrigger).addEventListener(
      'click',
      () => {
        this.next();
      },
    ...
    window.document.querySelector(this.prevTrigger).addEventListener(
      'click',
      () => {
        this.prev();
      },
  ...
  public defineWidthOfItems(width: number, resize): void {
    this.carouselElementRails.style.width =
      Math.floor(this.carouselElementItems.length * width) + 'px';
  ...
  public initInformations(): void {
    ...
    this.carouselElementRails = this.carouselElement.querySelector(
      '.carousel\\.rails'
    );
    this.carouselElementItems = this.carouselElement.querySelectorAll(
      '.carousel\\.rails > *'
    );
    if (parseInt(this.itemsByPages) > 1 && !this.isMobile) {
      let lastElement = this.carouselElementItems[0].cloneNode(true);
      let firstElement = this.carouselElementItems[
        this.carouselElementItems.length - 1
      ].cloneNode(true);
      this.carouselElementRails.appendChild(lastElement);
      this.carouselElementItems[0].parentNode.insertBefore(
        firstElement,
        this.carouselElementItems[0]
      );
      this.carouselElementItems = this.carouselElement.querySelectorAll(
        '.carousel\\.rails > *'
      );
