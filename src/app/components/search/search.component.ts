...
export class SearchComponent implements OnInit {
  ...
  public setInstallmentsOptionReset(): any {
    ...
    if(document.getElementById('installments')) document.getElementById('installments').innerHTML = opcoes_parcela;
  }
  ...
  public valueValidate(limit: number): any {
    return ( control => {
      ...
      value = value.split("");
      value = value.slice(1,value.length - 2).join("") + "." +value.slice(value.length - 2, value.length).join("");
  ...
  ngOnInit(): void {
    document.getElementById('installments').addEventListener('change', e => {
      this.simulator.get('installments').setValue((e.target as HTMLInputElement)?.value);
    }, false);
    this.setMask();
  }
